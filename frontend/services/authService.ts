...
// 创建带认证头的fetch请求，自动添加基础URL前缀
export async function authenticatedFetch(
  endpoint: string, // 例如: '/api/scans', '/api/auth/login'
  options: RequestInit = {}
): Promise<Response> {
  // 构建完整URL：使用标准化的基础URL + API端点
...
export async function verifyEmail(token: string): Promise<void> {
  const response = await fetch(buildUrl(`/api/auth/verify-email?token=${token}`), {
    method: "POST",
  })
...
export async function resendVerificationEmail(email: string): Promise<void> {
  const response = await fetch(buildUrl('/api/auth/resend-verification'), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
