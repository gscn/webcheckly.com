...
func GetUserTasks(userID string, limit, offset int) ([]*models.Task, error) {
	query := `
		SELECT id, user_id, status, target_url, options, language, ai_mode,
		       is_public, progress, modules, results, error,
		       created_at, updated_at, started_at, completed_at
...
func DeleteExpiredTasks() error {
	// 基础版：30天，专业版：90天，企业版：永久，免费用户：24小时
	query := `
		DELETE FROM tasks
...
// GetTaskCount 获取用户任务总数
func GetTaskCount(userID string) (int, error) {
	var count int
